...
void ArgParser::processSingleDash() {
    std::string arg = *commonIter;
    std::string::const_iterator sIter = arg.begin() + 1;    //skip the '-'
    ...
    while(sIter + 1 != arg.end()) {
        ...
        if(!isSwitch(*sIter))
            throw ArgParseException("Invalid switch: " + std::string(1, *sIter));

        switchesSet.push_back(std::string(1, *sIter));
        ++sIter;
    }
    ...
    if(isSwitch(*sIter))
        ...
        addDataOptPair(std::string(1, *sIter), data);
        ...
        throw ArgParseException("Invalid option: " + std::string(1, *sIter));
    }
...
void ArgParser::processDoubleDash() {
    std::string arg = *commonIter, data, opt;
    std::string::const_iterator sIter = arg.begin() + 2;     //skip the '--'
...
void ArgParser::addDataOptPair(const std::string &opt, const std::string &data) {
